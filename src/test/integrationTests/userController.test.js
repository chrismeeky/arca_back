...
describe('Integration tests for the user controller', () => {
  describe('Test general error handling and welcome message', () => {
    it('should send a "You are on arca NIG LTD but it looks like The page you are looking for does not exist" error when invalid URL is given', async () => {
      ...
      expect(response.body.message)
        .to.equal('You are on arca NIG LTD but it looks like The page you are looking for does not exist');
    });
  });
  describe('Test to pre-register a user', () => {
    ...
    it('should return an error when any user details is not given', async () => {
      ...
      expect(response.body).to.have.property('message');
      expect(response.body.message)
        .to.equal('User validation failed: lastName: Path `lastName` is required.');
    });
  });
